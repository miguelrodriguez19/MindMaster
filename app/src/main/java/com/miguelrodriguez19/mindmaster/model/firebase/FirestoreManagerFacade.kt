...
object FirestoreManagerFacade {
    ...
    // PASSWORDS
    fun saveGroup(
        group: PasswordGroupResponse, onSuccess: (PasswordGroupResponse) -> Unit
    ) = FAccountVaultManager.saveGroup(appContext, group) { onSuccess(it) }


    fun updateGroup(
        group: PasswordGroupResponse, onUpdated: (PasswordGroupResponse) -> Unit
    ) = FAccountVaultManager.updateGroup(appContext, group) { onUpdated(it) }

    fun loadAllGroups(
        onSuccess: (List<PasswordGroupResponse>) -> Unit
    ) = FAccountVaultManager.loadAllGroups(appContext) { onSuccess(it) }

    fun deleteGroup(
        group: PasswordGroupResponse, onDeleted: (PasswordGroupResponse) -> Unit
